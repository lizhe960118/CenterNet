...
class OneStageResNetDCN2(nn.Module):
    ...
    def __init__(self, depth, heads, deconv = True, out_indices=(0, 1, 2, 3), head_conv=64):
        super(OneStageResNetDCN2, self).__init__()
        self.inplanes = 64
        self.depth = depth
        self.deconv = deconv
    ...
    def forward(self, x):
        ...
        x = self.stage_one_deconv_layers(x)
        
        stage_one_z = {}
        for head in self.heads:
            stage_one_z[head] = self.__getattr__('stage_one_' + head)(x)
                
        return tuple([stage_one_z])

    def init_weights(self, pretrained=None):
        num_layers = self.depth
        print("init weights in resnet dcn")
       
        for name, m in self.stage_one_deconv_layers.named_modules():
    ...
    def _freeze_stages(self):
        ...
        for m in [self.stage_one_conv1, self.stage_one_bn1]:
...
#                 param.requires_grad = False
    ...
    def train(self, mode=True):
        super(OneStageResNetDCN2, self).train(mode)
        self._freeze_stages()
