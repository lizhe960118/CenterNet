...
class OneStageResNetDCN(nn.Module):
    ...
    def __init__(self, depth, heads, deconv = True, out_indices=(0, 1, 2, 3), head_conv=64):
        super(OneStageResNetDCN, self).__init__()
        self.inplanes = 64
        self.depth = depth
        self.deconv = deconv
        ...
        for i in range(len(_u_skip_in_layers)):
            ...
            self.stage_one_u_skip_layers.append(layer_name)
    ...
    def forward(self, x):
     ...
     #       print(x.shape)
            ...
            u_skip_conv_layer = u_skip(stage_one_downsample_outs[2 - i])
...
#                 param.requires_grad = False
    def train(self, mode=True):
        super(OneStageResNetDCN, self).train(mode)
        self._freeze_stages()
